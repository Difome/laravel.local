...
<div class="card">
  ...
  <div class="card-body">
    <form method="POST" action="{{ route('register') }}">
      ...
      <div class="form-group row">
        <label for="username" class="col-md-4 col-form-label text-md-right">{{ __('Пол') }}</label>
        <div class="col-md-6">
          <select class="custom-select" name="gender">
            <option value="male" @if (old('gender') == 'male') selected="selected" @endif>{{ __('Мужской') }}</option>
            <option value="female" @if (old('gender') == 'female') selected="selected" @endif>{{ __('Женский') }}</option>
          </select>
          @error('gender')
            <span class="invalid-feedback" role="alert">
              <strong>{{ $message }}</strong>
